...
def calculate_scenario_logic(df, cart_value):
    ...
    # ==========================================
    # LOGIC FILTER: ELIMINATE DOUBLE FREE
    # ==========================================
    # If Home is Free (Gap=0) AND Locker is Free (Gap=0), DROP IT.
    # We want to force a trade-off.
...
if full_design.empty:
    ...
    st.info("""
    **Design Notes:**
    * **Visual Deduplication:** Duplicate scenarios (where different backend prices resulted in the same "Free" offer) have been removed.
    * **Double-Free Filter:** Scenarios where both Home and Locker were free have been excluded to force trade-offs.
    * **Context Check:** Big Basket set to 750kr (instead of 850kr) to ensure valid Home Delivery thresholds.
    """)
