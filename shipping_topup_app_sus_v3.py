...
if q_idx >= len(df):
    ...
    # --- RENDER FUNCTION (UPDATED WITH DISTANCE) ---
    def render_option_row(title, subtitle, display_text, col_key, label_base, context_label, scenario_id, 
        ...
        with st.container():
            ...
            with c1:
                ...
                # Distance Logic
                if distance:
                    st.caption(f"{subtitle} • 📍 {distance}")
                else:
                    st.caption(subtitle)
            ...
            with c2:
                if " or Add " in display_text:
                    ...
                    # Top-Up Button (Neutral Color)
                    if st.button(f"➕ {val_only} (to 🛒)", key=f"btn_add_{col_key}", use_container_width=True):
